...
export function NearbyHealthUnitsModal({
  ...
  return (
    ...
    >
      {isLoading ? (
        <div className="rounded-[18px] border border-[#d8dde3] bg-[#f7f8f9] px-4 py-8 text-center text-sm text-[#697388]">
        ...
        <div className="max-h-[72vh] space-y-4 overflow-y-auto pr-1">
          {items.map((unit) => {
            ...
            return (
              <article
                key={unit.id}
                className="rounded-[18px] border border-[#d9dede] bg-white px-4 py-4"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-[18px] font-semibold text-[#02051f]">
                      {unit.name}
                    </h3>
                    <p className="mt-1 text-[14px] text-[#7e8599]">
                      {unit.unitType}
                    </p>
                  </div>

                  <span className="rounded-full bg-[#d8f5e9] px-3 py-1 text-[13px] font-medium text-[#2d8f73]">
                    {formatDistance(unit.distanceKm)}
                  </span>
                </div>

                <div className="mt-4 flex items-start gap-2 text-[15px] text-[#5c6579]">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex h-11 items-center justify-center gap-2 rounded-xl border border-[#d8dde3] bg-white text-[16px] font-medium text-[#02051f] transition hover:bg-[#f7f8f9]"
                  >
                  ...
                  {callUrl ? (
                    ...
                    <button
                      type="button"
                      disabled
                      className="flex h-11 items-center justify-center gap-2 rounded-xl border border-[#e4e7ea] bg-[#f5f7f9] text-[16px] font-medium text-[#9aa1b5]"
                    >
                      Ligar
                    </button>
                  )}
